...
class Shopify{
	...
	public function getAccessToken($code = '')
	{
		...
		$data = $this->call(array('METHOD' => 'POST', 'URL' => 'https://' . $this->_API['SHOP_DOMAIN'] . '/admin/oauth/access_token', 'DATA' => $dta), FALSE);
    ...
    public function call($userData = array(), $verifyData = TRUE)
    {
        ...
        foreach($info as $k => $header)
        {
	        ...
	        $delimiter_pos = strpos($header, ':');
	        $key = substr($header, 0, $delimiter_pos);
	        $val = substr($header, $delimiter_pos + 1);
